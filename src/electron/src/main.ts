...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: resolve(__dirname, "renderer.js"),
      ...
      submenu: [
        {
          ...
          click() {
            ...
            child.removeMenu();
            child.loadFile(resolve(__dirname, "about.html"));
  ...
  ipcMain.on("checkConnectStatus", async (event, args) => {
    try {
      const status = await internetAvailable({
        domainName: "api.github.com",
        timeout: 1000
      });
      console.log("status", status);
      event.sender.send("checkConnectStatus-response", "connected");
    } catch (err) {
      event.sender.send("checkConnectStatus-response", "disconnected");
    }
  });
